...
void main(){
	...
	gl_Position = unused * vec4(doTransform(vec2(halfTileSize.x,-halfTileSize.y), AffineT, gl_in[0].gl_Position.xy),0.f,1.f);
	EmitVertex();
	texposition = texTopLeft;
	gl_Position = unused * vec4(doTransform(-halfTileSize, AffineT, gl_in[0].gl_Position.xy),0.f,1.f);
	EmitVertex();
